...
# Note that executing the code below is of course dependent on your directory
# structure and Plink installation. Directory structure from
# https://github.com/Skirnir3141/AppleProject/ should be maintained. All files
# necessary to run this are uploaded on github except for the
# abc_combined_maf001_sort_vineland_imputed .map and .ped files, which need to
# be downloaded from https://datadryad.org/stash/dataset/doi:10.5061/dryad.zkh1893cd
# and extracted to the ./Data/Snps folder. Plink v1.90 should be downloaded from
# the cog-genomics.org website (https://www.cog-genomics.org/plink2/) and
# installed in the /usr/local/bin folder on a computer running linux and made
...
These structural and technological developments facilitated a large globally connected industry, but they may have come at the cost of genetic diversity. Pre-industrialization, farmers would have presumably combined whatever cultivars were locally available to them and the fragmented nature of the market would have meant high global diversity. But, genetic research has shown that many accessions in collections today are clonally related to just a few commercially dominant cultivars such as McIntosh (Migicovsky et al., 2021).
...
Per Figure 1, results showed that cultivar relatedness increased in each period. Looking at accession relatedness using IBD, the percentage of accessions that had at least one relationship increased from `r scales::percent(round(nrow(as_tibble(activate(p1.graph, nodes))) / nrow(p1.nodes), digits = 2))` in P1 to `r scales::percent(round(nrow(as_tibble(activate(p2.graph, nodes))) / nrow(p2.nodes), digits = 2))` in P2 to `r scales::percent(round(nrow(as_tibble(activate(p3.graph, nodes))) / nrow(p3.nodes), digits = 2))` in P3. Plotting networks of accessions with at least one relationship, we can see that cultivars became more related to each other in each period as indicated visually by increased number of connections and density of nodes. Extracting the global efficiency of each graph confirms this. From `r scales::percent(round(igraph::global_efficiency(p1.graph), digits = 2))` in P1, it increased to `r scales::percent(round(igraph::global_efficiency(p2.graph), digits = 2))` in P2 and then `r scales::percent(round(igraph::global_efficiency(p3.graph), digits = 2))` in P3.

However, despite increased relatedness across periods, we can also see in Figure 1 that the first two principle components of the genetic distance PCA did not suggest any difference in the genetic diversity of accessions between periods. This can be concluded because accessions from all periods occupy contiguous graph space.

``` {r graph_networks, echo=FALSE, fig.align="center", fig.height=3, fig.cap="Network graphs of related accessions and PCA of SNPs across all accessions.", error=FALSE, warning=FALSE, message=FALSE}
